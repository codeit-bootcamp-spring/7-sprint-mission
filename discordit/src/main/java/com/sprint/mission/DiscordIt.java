...
public class DiscordIt {
    ...
    private void messageSearch() {
        ...
        switch (getInput(3)) {
            ...
            case 3 -> {
                UUID receiverId = displayChannelAndSelect(channelService.getAllChannels());
    ...
    private void sendDirectMessage() {
        ...
        while (true) {
            ...
            messageService.sendMessage(
                    userService.getUserById(loginId),
                    userService.getUserById(receiveId),
                    message
            );
        }
    }
    ...
    private void leaveChannel() {
        ...
        printLine();
        List<UUID> registeredChannels = channelService.getRegisteredChannels(userService.getUserById(loginId));
        System.out.print("나갈 채널을 선택해주세요. >>");
        UUID channel = displayChannelAndSelect(registeredChannels);
        channelService.deleteMember(channel, userService.getUserById(loginId));
        System.out.println("채널에서 나왔습니다.");
    }

    private void enterChannel() {
        printLine();
        List<UUID> channels = channelService.getRegisteredChannels(userService.getUserById(loginId));
        if (channels.isEmpty()) {
    ...
    private void channelMessageSend(UUID uuid) {
        User sender = userService.getUserById(loginId);
    ...
    private void registerChannel() {
        List<UUID> channels = channelService.getNotRegisteredChannels(userService.getUserById(loginId));
        ...
        channelService.addMember(selectedChannel, userService.getUserById(loginId));
