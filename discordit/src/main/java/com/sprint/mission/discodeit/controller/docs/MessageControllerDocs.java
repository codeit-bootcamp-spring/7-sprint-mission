...
import com.sprint.mission.discodeit.dto.page.PageRequest;
...
public interface MessageControllerDocs {
    @Operation(
            summary = "채널별 메시지 조회",
            description = "특정 채널의 메시지를 페이징하여 조회합니다"
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공")
    })
    ResponseEntity<PageResponse<?>> getByChannelId(@Valid @RequestParam UUID channelId, @RequestParam PageRequest pageable);
    ...
    ResponseEntity<MessageDto> send(@Valid @RequestPart(name = "messageCreateRequest") MessageCreateRequest messageCreateRequest, @RequestPart(name = "attachments", required = false) List<MultipartFile> attachments);
    ...
    ResponseEntity<MessageDto> edit(@RequestParam UUID messageId, @Valid @RequestBody MessageEditRequest request);
}