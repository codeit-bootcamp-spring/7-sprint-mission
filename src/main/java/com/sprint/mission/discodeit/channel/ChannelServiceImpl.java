...
public class ChannelServiceImpl extends BaseServiceImpl<Channel, UUID, ChannelRepository> implements ChannelService {
    ...
    public ChannelResponseDTO create(ChannelRequestDTO requestDTO) {
        ...
        String topic = requestDTO.topic();
        boolean isPrivate = requestDTO.isPrivate();
        ...
        return ChannelResponseDTO.from(  save(newChannel));
    }
    ...
    public List<ChannelResponseDTO> findAllChannelsBySettings(ChannelRequestDTO requestDTO) {
        ...
        return repository.findAllChannelsBySettings(channelName, channelType, topic).stream()
    ...
    @Override
    public List<ChannelResponseDTO> findAllChannelsBySettingsNonDel(ChannelRequestDTO requestDTO) {
        String channelName = requestDTO.ChannelName();
        String topic = requestDTO.topic();
        ChannelType channelType = requestDTO.channelType();
        return repository.findAllChannelsBySettingsNonDel(channelName, channelType, topic).stream()
                .map(ChannelResponseDTO::from)
                .toList();
    }
}