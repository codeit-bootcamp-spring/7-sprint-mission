...
/**
 * 사용자 정보를 담는 핵심 도메인 엔티티입니다. 이 클래스는 사용자의 데이터뿐만 아니라, 자신의 상태를 직접 관리하는 비즈니스 로직을 포함합니다. (풍부한 도메인 모델,
 * Rich Domain Model)
 */
@Getter
@Setter
public class User extends BaseEntity<UUID> {
  ...
  /**
   * 외부에서 `new User()`를 통해 불완전한 객체를 생성하는 것을 막기 위해 생성자를 protected로 선언합니다. 객체 생성은 반드시 `create()` 정적
   * 팩토리 메서드를 통해서만 이루어져야 합니다.
   */
  private User() {
  ...
  /**
   * User 객체를 안전하게 생성하는 정적 팩토리 메서드입니다. 필요한 모든 정보를 받아 유효성 검사를 거친 후, 완전한 상태의 객체를 생성하여 반환합니다.
   *
   ...
   */
  public static User createUser(String username, String password, String email, String nickname,
      String phoneNum) {
  ...
  public void tempUpdateProfile(String nickname, String email, String password) {
    boolean isChanged = false;
